...
ifeq ($(CROSSBUILD),ipados)
	...
	DEPS_CMAKE_CONFIG_ARGS:=-DCMAKE_SYSTEM_NAME=iOS -DCMAKE_OSX_ARCHITECTURES=arm64 $(DEPS_CMAKE_CONFIG_ARGS)
else
	DEPS_CMAKE_CONFIG_ARGS:=$(DEPS_CMAKE_CONFIG_ARGS)
endif
...
deps: .FORCE
	...
	@cmake -H"$(DEPS_SOURCE_DIR)" \
	       -B"$(DEPS_BUILD_DIR)" \
		   -DHARVEST_TARGET=$(DEPS_INSTALL_DIR) $(DEPS_CMAKE_CONFIG_ARGS)
